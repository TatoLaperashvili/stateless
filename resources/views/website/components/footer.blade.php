                ...
                <div class="row row-reverse">
                    <div class="col-xxl-6 col-lg-6 col-md-6 col-sm-12">
                        <address class="footer-contact">
                            ...
                            <a href="{{settings('F_address_Map')}}" target="_blank">
                    ...
                    <div class="col-xxl-6 col-lg-6 col-md-6 col-sm-12">
                        <div class="footer-form">
                            ...
                            <h2>{{ trans('website.Subscribe_news') }}</h2>
                            <form method="POST" action="/{{ app()->getLocale() }}/subscribe">
                                @csrf
                                <input type="text" name="name" placeholder="{{trans('admin.name')}}"  >
                                <input type="text" name="surname" placeholder="{{trans('admin.Surname')}}" >
                                <div class="input-lab-box input-lab-width2">
                                    <label>{{trans('admin.email')}}</label>
                                    <input type="email" name="email" placeholder="{{trans('admin.email')}}" required>
                                </div>
                                @if(Session::has('subscribe'))
                                <p class="alert alert-footer">{{ Session::get('subscribe') }}</p>
                                @endif
                                <button class="ff-bt">
                                    {{ trans('website.Subscribe') }}
                                </button>
                            </form>
                        </div>
                    </div>
...
@if(app()->getLocale() == 'ka')
                            <img src="/website/assets/img/logo2.svg" alt="svg">
  @else
  <img src="/website/assets/img/Rights Georgia logo Eng Geo-04.png" alt="logo_en" style="width: 80%">
@endif
                        </div>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-xxl-4 col-lg-4 col-md-4">
                                ...
                                <div class="footer-links">
                                    <h2>  <a href="/{{ $fsection->getFullSlug() }}">{{ strtoupper($fsection[app()->getlocale()]->title) }}</a></h2>
                                    ...
                                    <a href="/{{ $subSec->getFullSlug() }}">{{ strtoupper($subSec[app()->getlocale()]->title) }}</a>
                                     ...
                                     @foreach ($subSec->children as $grandchildren )
                                     <a href="/{{ $grandchildren->getFullSlug() }}">{{ strtoupper($grandchildren[app()->getlocale()]->title) }}</a> 
                                     @endforeach
                                    @endforeach
                                    @endif
